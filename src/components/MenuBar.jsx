...
// Added user, onLoginClick, and onLogout props
const MenuBar = ({ filters, onFilterChange, onLocationClick, onMenuToggle, user, onLoginClick, onLogout }) => {
    const [showBuyDropdown, setShowBuyDropdown] = useState(false);

    return (
        <div className="menu-bar-container">
            <div className="menu-bar-left">
                {/* Logo links to the search page */}
            ...
            <div className="menu-bar-right">
                <button className="menu-item post-property">Post Your Property</button>

                {user ? (
                    <>
                        <span className="menu-item user-info" style={{ color: '#7c3aed', fontWeight: '600' }}>Hi, {user.username}!</span>
                        <button className="menu-item" onClick={onLogout} style={{ color: '#ef4444' }}>
                            Log out
                        </button>
                    </>
                ) : (
                    <>
                        ...
                        <button className="menu-item" onClick={onLoginClick} style={{ backgroundColor: '#ede9fe', color: '#7c3aed', fontWeight: '600' }}>
                            Log in
                        </button>
                    </>
                )}
